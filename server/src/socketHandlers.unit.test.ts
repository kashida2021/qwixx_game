...
jest.mock("./roomUtils", () => {
 const generateUniqueRoomId = jest.fn().mockReturnValue("1234");
...
describe("socket event handler test", () => {
 ...
 describe("socket event handlers with 2 clients", () => {
  ...
  test.only("client sockets can join same room", (done) => {
  ...
  test("clients can join different rooms", (done) => {
   ...
   clientSocket1.on("player_joined", checkClientsInRooms);
   clientSocket2.on("player_joined", checkClientsInRooms);
  });
