...
describe("gameboard temp test", () => {
  ...
  describe("Locking rows", () => {
    test.each([
      [rowColour.Red, [2, 3, 4, 5, 6, 12], { red: true, yellow: false, green: false, blue: false }],
      [rowColour.Yellow, [2, 3, 4, 5, 6, 12], { red: false, yellow: true, green: false, blue: false }],
      [rowColour.Green, [12, 11, 10, 9, 8, 2], { red: false, yellow: false, green: true, blue: false }],
    ...
    test.each([
      [rowColour.Red, [2, 3, 4, 5, 6], { red: false, yellow: false, green: false, blue: false }],
      [rowColour.Yellow, [2, 3, 4, 5, 6], { red: false, yellow: false, green: false, blue: false }],
      [rowColour.Green, [12, 11, 10, 9, 8], { red: false, yellow: false, green: false, blue: false }],
      [rowColour.Blue, [12, 11, 10, 9, 8], { red: false, yellow: false, green: false, blue: false }],
    ])("Can't lock %s row if row's final number hasn't been marked", (row, numbers, expected) => {
      numbers.forEach(num => testGameCard.markNumbers(row, num))

      const res = testGameCard.lockRow(row)
      expect(res).toEqual({ success: false, errorMessage: "Didn't satisfy conditions to lock a row." })
